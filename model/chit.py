...
class DepthChiTransformer(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if not self.multiscale:
            ...
            depth[("depth", 3)] = self.depth_estimate(depth['head4'])
            depth[("depth", 2)] = self.depth_estimate(depth['head3'])
            depth[("depth", 1)] = self.depth_estimate(depth['head2'])
            depth[("depth", 0)] = self.depth_estimate(depth['head1'])
